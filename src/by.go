...
var(
	...
	crBy  = []string{"Creation Date:"}
	udBy  = []string{"Updated Date:"}
	edBy  = []string{"Expiration Date:"}
	rrBy  = []string{"Registrar:"}
	rtBy  = []string{"Registrant:", "Org:"}
	rtcBy = []string{"Country:" }

)
/*
	There is placed template parser for sites with .by top level domain.
*/

func ByTemplate(html []byte, target string) *Whois {
	...
	for _, element := range submatchall {
		...
		for _, str := range s {
			...
			if strings.Contains(str, crBy[0]) {

				value := RuTrimmer(str, crBy)
				...
				// No need in parseDate because there is only y-m-d without hours
				//value = parseRuDate(value)
				wi.RegistrationDate = value

			}
			if strings.Contains(str, udBy[0]) {

				value := RuTrimmer(str, udBy)
			...
			if strings.Contains(str, edBy[0]) {

				value := RuTrimmer(str, edBy)
			...
			if strings.Contains(str, rrBy[0]) {

				value := RuTrimmer(str, rrBy)
				value = strings.TrimPrefix(value, " ")
				wi.Registrar = value
			}
			if strings.Contains(str, rtBy[0]) || strings.Contains(str, rtBy[1]) {

				value := RuTrimmer(str, rtBy)
			...
			if strings.Contains(str, rtcBy[0]) {
				value := strings.Replace(str, rtcBy[0],"", 1)
			...
			if !wasThereAlready {
				if !strings.Contains(string(html), rtBy[0]) || !strings.Contains(string(html), rtBy[1]) || !strings.Contains(string(html), rtBy[2]) {
					value, err := GetWhoisHistoryFree(target, 4)
					if err != nil {
						fmt.Println(err)
					}
