...
var(
	...
	crRu  = []string{"Creation Date:", "created:"}
	udRu  = []string{"Updated Date:", "updated:", "Last updated on"}
	edRu  = []string{"Registry Expiry Date:", "Registrar Registration Expiration Date:", "paid-till:"}
	rrRu  = []string{"Registrar:", "registrar:", "Registrar Name:" }
	rtRu  = []string{"Registrant Name:", "registrant:", "owner:"}
	rtcRu = []string{"Registrant Country:" }

)
/*
	There is placed template parser for RIPN Whois Service
	that is for web sites in .ru top-level domain
 */

func RuTemplate(html []byte, target string) *Whois {
	...
	for _, element := range submatchall {
		...
		for _, str := range s {
			...
			if strings.Contains(str, crRu[0]) || strings.Contains(str, crRu[1]) {

				value := RuTrimmer(str, crRu)
				...
				value = parseRuDate(value)
				wi.RegistrationDate = value

			}
			if strings.Contains(str, udRu[0]) || strings.Contains(str, udRu[1]) || strings.Contains(str, udRu[2]) {

				value := RuTrimmer(str, udRu)
			...
			if strings.Contains(str, edRu[0]) || strings.Contains(str, edRu[1]) || strings.Contains(str, edRu[2]) {

				value := RuTrimmer(str, edRu)
			...
			if strings.Contains(str, rrRu[0]) || strings.Contains(str, rrRu[1]) || strings.Contains(str, rrRu[2]) {

				value := RuTrimmer(str, rrRu)
				value = strings.TrimPrefix(value, " ")
				wi.Registrar = value
			}
			if strings.Contains(str, rtRu[0]) || strings.Contains(str, rtRu[1]) || strings.Contains(str, rtRu[2]) {

				value := RuTrimmer(str, rtRu)
			...
			if strings.Contains(str, rtcRu[0]) {
				value := strings.Replace(str, rtcRu[0],"", 1)
			...
			if !wasThereAlready {
				if !strings.Contains(string(html), rtRu[0]) || !strings.Contains(string(html), rtRu[1]) || !strings.Contains(string(html), rtRu[2]) {
					value, err := GetWhoisHistoryFree(target, 4)
					if err != nil {
						fmt.Println(err)
					}
...
// parseRuDate parses 0Z timezone to +3 UTC
func parseRuDate(date string) string {
